 ...
 * Version: 1.4
 * Description: Design Photo, Video, Link, Image Gallery with CSS3 Transitions and Animations & Display With Lightbox
 * Author: WebHunt Infotech
 * Author URI: http://webhuntinfotech.com
 * Plugin URI: http://demo.webhuntinfotech.com/demo/#pvgmp
 */
...
class PVGM {
	...
	public function add_all_pvgm_meta_boxes() {
		...
		add_meta_box(__('Pro Features', 'PVGM_TEXT_DOMAIN') , __('Pro Features', 'PVGM_TEXT_DOMAIN'), array(&$this,'pvgm_pro_features'), 'pvgm_gallery', 'side', 'low');
	}
	...
	?>
		<ul style="">
			<li class="plan-feature">(1) <?php _e('Responsive Design','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(2) <?php _e('10 Beautiful Transition Effects','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(3) <?php _e('Three Type Gallery Layout','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(4) <?php _e('Isotope/Masonry Effects','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(5) <?php _e('Unlimited Hover Color','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(6) <?php _e('Youtube/Vimeo Video','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(7) <?php _e('Extrenal Link','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(8) <?php _e('Rectangular/Circular Image Design Layout','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(9) <?php _e('All Gallery Shortcode','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(10) <?php _e('Each Gallery has Unique Shortcode','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(11) <?php _e('500+ of Font Style','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(12) <?php _e('2 Types Of Lightbox','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(13) <?php _e('Drag and Drop image Position','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(14) <?php _e('Multiple Image uploader','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(15) <?php _e('Shortcode Button on post or page','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(16) <?php _e('10 Types of Hover Color Opacity','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(17) <?php _e('Unique settings for each gallery','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(18) <?php _e('Hide/Show gallery Title and label','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(19) <?php _e('Hide/Show gallery Discription and View More Button','PVGM_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(20) <?php _e('Google Fonts','PVGM_TEXT_DOMAIN'); ?></li>
		</ul>
	<?php
	}
    ...
    public function PVGM_generate_add_image_meta_box_function($post) { ?>
			<div class="" style="padding:20px;text-align: center;">
			  <a  href="http://demo.webhuntinfotech.com/documentation/photo-video-gallery-master-pro/" target="_new" class="btn-web button-1"><?php _e('Documention','PVGM_TEXT_DOMAIN'); ?></a>
			  <a href="http://demo.webhuntinfotech.com/demo/#pvgmp" target="_new" class="btn-web button-2"><?php _e('View Live Demo','PVGM_TEXT_DOMAIN'); ?></a>
			  <a href="http://webhuntinfotech.com/amember/signup/pvgmp" target="_new" class="btn-web button-3"><?php _e('Upgrade To Pro','PVGM_TEXT_DOMAIN'); ?></a>
			</div>
		<div >
			...
			<input id="PVGM_delete_all_button" class="button" type="button" value="Delete All" rel="">
			<input type="hidden" id="PVGM_wl_action" name="PVGM_wl_action" value="PVGM-save-settings">

			<!--Add New Image Button-->
			<div class="pvgm-image-entry add_pvgm_new_image" id="pvgm_gallery_upload_button" data-uploader_title="Upload Image" data-uploader_button_text="Select" style="margin:10px;">
					...
					foreach($WPGP_AllPhotosDetails as $WPGP_SinglePhotoDetails) {
						...
						<li class="pvgm-image-entry" id="pvgm_img">
							...
							<div class="pvgm-admin-inner-div2" >
								...
								<p>
									<div class="pvgm_label"><label ><?php _e('Label','PVGM_TEXT_DOMAIN')?></label></div>
								...
								<p>
									<label class="pvgm_label"><?php _e('Link','PVGM_TEXT_DOMAIN')?></label>
								...
								<p>
									...
									<textarea id="PVGM_image_descp[]" name="PVGM_image_descp[]" class="pvgm_textarea" placeholder="Enter Image Description"><?php echo esc_textarea($description); ?></textarea>
								</p>
							</div>
						</li>
						<?php
			...
			if(isset($_POST['PVGM_image_url'])){
				...
				if($TotalImages) {
					for($i=0; $i < $TotalImages; $i++) {
						...
						$ImagesArray[] = array(
							'PVGM_image_label' => sanitize_text_field( $image_label ),
							...
							'PVGM_image_descp' => esc_textarea( $description )
						);
					}
	...
	public function PVGM_settings_meta_save($PostID) {
	  if(isset($PostID) && isset($_POST['PVGM_Show_Gallery_Title'])){
		...
		$PVGM_Button_Text	   		= sanitize_text_field( $_POST['PVGM_Button_Text'] );
...
/**
 * Simple Profolio Gallery Short Code [PVGM].
 */
...
function pvgm_add_pvgm_custom_button($context) {
  ...
  href="#TB_inline?width=400&inlineId='.$container_id.'">
	...
	'. __("Portfolio Gallery Shortcode",'PVGM_TEXT_DOMAIN').'
	</a>';
  return $context;
}
...
function pvgm_add_pvgm_inline_popup_content() {
	...
	<div id="PVGM" style="display:none;">
	  <h3><?php _e('Select Responive Portfolio Gallery To Insert Into Post','PVGM_TEXT_DOMAIN');?></h3>
	  <?php
